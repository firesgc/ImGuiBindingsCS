...
public sealed class StructGenerator
{
    ...
    public string Generate(List<StructItem> structs)
    {
    ...
    private void GenerateField(CodeWriter w, StructField field, string structName)
    {
        ...
        var trailing = w.FormatTrailingComment(field.Comments?.Attached);
        ...
        if (field.Type.TypeDetails?.Flavour == "function_pointer")
        {
            ...
            w.WriteLine($"public nint {csFieldName};{trailing}");
        ...
        {
            GenerateArrayField(w, field, csFieldName, trailing, structName);
            return;
        }

        // Regular field
        var csType = _typeResolver.Resolve(field.Type);
        w.WriteLine($"public {csType} {csFieldName};{trailing}");
    }

    private void GenerateArrayField(CodeWriter w, StructField field, string csFieldName, string trailing, string structName)
    {
        ...
        if (TryResolveBounds(bounds, out var size))
        {
            if (TypeMapper.IsFixedBufferCompatible(elementType))
            {
                ...
                w.WriteLine($"public fixed {elementType} {csFieldName}[{size}];{trailing}");
            }
            else
            {
                ...
                w.WriteLine($"public {inlineArrayName} {csFieldName};{trailing}");
            }
        }
        else
        {
